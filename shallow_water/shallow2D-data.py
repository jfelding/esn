...
xs = np.linspace(0,Lx,nx)[None,:]
ys = np.linspace(0,Ly,ny)[:,None]
...
smooth_dam_break((30,30),x0=(Lx/2,Ly/2),height=1)
...
t,t0 = 0,0
T = 2000
etadata = np.zeros([T]+list(eta.shape))
...
for j in range(T):
    ...
    if (t-t0>10): # Check water volume every 10 simulation seconds
        t0 = t
        print(f"at step {j}, {round(t,2)} seconds, water volume in eta is {round(dx*dy*np.sum(eta[M,M]),8)}")
        
    etadata[j] = eta

np.save("shallow2D.npy",etadata)
